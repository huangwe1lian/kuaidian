; (function($) {
	$.simpleCanleder = function(box, options, callback){
                ...
                $(document).click(function(e){
                    ...
                    if(canlederBox.is(":visible") && $box && e.target != $box[0] && isCanlederBox.size() <= 0){
                        ...
                        if(pointX > left && pointY > top &&
                            ...
                            canlederBox.hide().remove();
                        }
                    }
                });
			}
			
			if(box.parents(".unitBox").height() - box.position().top - box.parents(".unitBox").find(".pageHeader").height() < 180){
				canlederBox.css({"top" : box.position().top+box.parents(".unitBox").find(".pageHeader").height()-canlederBox.height()+box.parents(".unitBox").find(".pageHeader").height()+8, "left": boxOffset.left});
			}else{
    ...
    $.fn.extend({
        simpleCanleder: function(options, callback) {
            options = $.extend({},options);
            this.each(function() {
				new $.simpleCanleder(this, options, callback);
			});
			return this;
        }
    });
